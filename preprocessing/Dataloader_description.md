...
| `distribution`         | Tuple[str, Union[int, float]] | 数据分布策略，第一个元素为策略类型('default'或'dirichlet')，第二个元素为参数：'default'时为旧版的k值；'dirichlet'时为alpha值(越小分布越不均衡) | ('default', 1) |
