...
class FedDPAdvClient(FedClient):
    def __init__(self, name, epoch, dataset_id, model_name, dataset_info: list|tuple,
                 ...
                 clip_norm=0.8,
                 # 2. 优化对抗训练参数（关键调整点）
                 adv_epsilon_init=0.025,  # 降低初始扰动（原0.03），避免干扰模型初期拟合
                 adv_lambda=0.002,
                 adv_pgd_steps=5):  # 增加PGD步数（原4），生成更精准的对抗样本
        super().__init__(name, epoch, model_name, dataset_info)
        # 2.1 DP参数（与FedDp完全一致，不可修改）
        ...
        self.current_global_round = 0  # 与FedDp一致的轮次计数器

    def _compute_noise_scale(self):
        """3. 保持与FedDp完全一致的DP噪声计算，不可修改"""
    ...
    def _get_adaptive_params(self):
        """4. 优化对抗扰动衰减：减缓衰减速度，保持足够扰动强度"""
        if self.current_global_round < 30:
            # 衰减系数从0.8→0.5，让epsilon下降更平缓；基数从0.3→0.4，提升扰动基数
            epsilon = (self.adv_epsilon_init * 0.4) * np.exp(-self.adv_lambda * 0.5 * self.current_global_round)
            epsilon = max(epsilon, 0.008)  # 提高最小扰动（原0.006），确保后期仍有对抗效果
        else:
            # 30轮后衰减系数从0.2→0.3，进一步减缓衰减
            epsilon = (self.adv_epsilon_init * 0.4) * np.exp(-self.adv_lambda * 0.5 * 30) * np.exp(
                -self.adv_lambda * 0.3 * (self.current_global_round - 30))
            epsilon = max(epsilon, 0.006)  # 稳定后期扰动
        alpha = epsilon / self.adv_pgd_steps  # 步长与PGD步数匹配（步数已改为5）
        use_pgd = self.current_global_round >= 6  # 保持提前启动PGD，不修改
    ...
    def _generate_adv_samples(self, x, y):
        ...
        if use_pgd:
            # PGD步长系数从0.95→0.98，减少过度扰动，提升样本质量
            step_coeff = 0.95 if self.current_global_round < 30 else 0.98
            for _ in range(self.adv_pgd_steps):  # 步数已改为5
                ...
                x_adv = torch.clamp(x_adv, x - epsilon, x + epsilon)  # 约束扰动范围
                x_adv = torch.clamp(x_adv, 0.0, 1.0)  # 新增：确保像素值在0-1，避免样本异常
                x_adv = x_adv.detach().requires_grad_(True)
        else:
            # FGSM阶段：扰动系数从0.6→0.7，提升初始对抗效果
            output = self.model(x_adv)
            loss = loss_func(output, y)
            self.model.zero_grad()
            loss.backward()
            x_adv = x_adv + 0.7 * epsilon * x_adv.grad.sign()
            x_adv = torch.clamp(x_adv, 0.0, 1.0)

        # 6. 优化输入噪声：平衡隐私与样本质量
        grad_sigma = self._compute_noise_scale()
        # 输入噪声比例：前20轮0.008（原0.01），20轮后0.005（原0.003），减少噪声干扰
        input_sigma_ratio = 0.008 if self.current_global_round < 20 else 0.005
    ...
    def train(self):
        ...
        # 7. 优化器与学习率调度：与FedDp完全一致，确保DP基础相同
        ...
        for epoch in range(self._epoch):
            for step, (x, y) in enumerate(train_loader):
                ...
                # 10. 优化对抗样本混合比例：从“按步取模”改为“按批随机混合”，更稳定
                if self.current_global_round < 10:
                    mix_ratio = 0.125  # 前10轮：1/8样本为对抗样本，逐步适应
                elif self.current_global_round < 30:
                    mix_ratio = 0.167  # 10-30轮：1/6样本为对抗样本，提升鲁棒性
                else:
                    mix_ratio = 0.2  # 30轮后：1/5样本为对抗样本，稳定提优
                # 随机混合对抗样本与正常样本（每批都有混合，避免波动）
                mask = torch.rand(b_x.shape[0], device=self._device) < mix_ratio
                mask = mask.unsqueeze(1).unsqueeze(2).unsqueeze(3)  # 扩展维度匹配图像
                train_x = torch.where(mask, x_adv, b_x)

                # 11. 模型训练（与FedDp一致，仅输入改为混合样本）
                self.model.train()
